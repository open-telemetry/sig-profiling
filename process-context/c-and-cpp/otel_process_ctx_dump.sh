...
echo "  otel_process_monotonic_published_at_ns : $monotonic_published_at_ns"
...
if command -v protoc >/dev/null 2>&1; then
  ...
  dd if="/proc/$pid/mem" bs=1 count="$payload_size" skip=$((16#$payload_ptr_hex)) status=none | protoc --decode=opentelemetry.proto.processcontext.v1development.ProcessContext process_context.proto resource.proto common.proto
else
  echo
